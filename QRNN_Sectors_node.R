...
node.select <- data.frame(matrix(NA,nrow = 5,ncol = 10))
colnames(node.select) <- c("Node","Materials","Energy","Financial","Industrials",
...
matplot(node.select[,1],node.select[,2:9],pch=c(1:9),col = "black",
        xlab = "Number of Neurons", ylab = "MAPE")
legend('topright', Target.list, pch=1:9, cex=.65)
