...
#Plot the training data__________________________________________________________

# fit.model.qrf.predict <- predict(qrf,newdata= modelDF[,1:length(modelDF)-1], what=mean)
# fit.model.qrf.actual <- modelDF$Target
# fit.model.plot.data <- data.frame(cbind(fit.model.qrf.predict,fit.model.qrf.actual))
# colnames(fit.model.plot.data) <- c("Predict", "Actual")
# fit.model.date <- head(date_seq,-validate_months)
# plot(fit.model.date,fit.model.plot.data$Actual,col="red", main = paste("Fit the model using training
#                                                              data for ",Target.list[target])
#      ,xlab = "Date", ylab = "Price",type="l", lwd = 2)
# lines(fit.model.date,fit.model.plot.data$Predict,type="c",lwd=2)
# legend('topright', c("Actual","Predict"), col=1:ncol(fit.model.plot.data), lty=1, cex=.65)
...
jpeg(filename = paste("QRF_",Target.list[target],".jpeg"))
